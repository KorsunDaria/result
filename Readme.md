## <span style="color: #1565c0;">1. Расширение нагрузочного тестирования pg_vector и его расширений </span>

**1) Рыночные расширения** — VectorChord (RaBitQ, сжатие 4x и 8x).

**2) Рыночные расширения** — бенчмарк альтернатив: VectorChord (RaBitQ, сжатие 4x и 8x) и pgvectorscale.
...
- **CPU:** Intel Core i7-8565U @ 1.80GHz (4 ядра / 8 потоков)
...
* **Параллелизм:** Рост числа воркеров замедляет сборку на данном компе (аппаратный лимит/контеншн). 
* **Сжатие (RaBitQ):** Уменьшает размер индекса, но замедляет сборку и поиск, если данные помещаются в RAM. Рекомендация применять только при нехватке памяти.
* **RAM-порог:** Резкий скачок производительности при выходе из swap-зоны (8 ГБ для 500K × 1536 dim). Формула для планирования: `RAM = Размер_индекса × 2.5 - 3.5`.
* **Recall под фильтром:** Повышение `ef_search` не помогает. Решение: `iterative_scan = relaxed_order` c jujdjhrfvb.

## <span style="color: #1565c0;">5. Направление 1. VectorChord RaBitQ</span>

**Цель:**

- в оригинальном исследовании создание индекса упиралось в верхний потолок RAM, что делало не возмодным работу на больших датасетах
...
![50_1](./50k/metrics_test1_pgvector_hnsw_cachemiss_v2.png)
![50_2](./50k/metrics_test2_vectorchord_rabitq8_cachemiss_v2.png)
![50_3](./50k/metrics_test3_vectorchord_rabitq4_cachemiss_v2.png)

**IPC**

![50_1](./50k/metrics_test1_pgvector_hnsw_ipc_v2.png)
![50_2](./50k/metrics_test2_vectorchord_rabitq8_ipc_v2.png)
![50_3](./50k/metrics_test3_vectorchord_rabitq4_ipc_v2.png)
...
## <span style="color: #1565c0;">2. Направление 3. Параллелизм при сборке и аппаратные ограничения</span>
